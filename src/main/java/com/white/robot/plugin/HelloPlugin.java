...
import com.white.robot.service.PreciseService;
...
public class HelloPlugin extends BotPlugin {
        ...
        if (messageChain.size() > 0) {
            ...
            if (message.getType().equals("text")) {
                ...
                if ("hello".equals(text)) {
                    String mes = preciseService.getByKeyword("#list").getResponse();
                    bot.sendPrivateMsg(event.getUserId(), mes, false);
    ...
    public int onGroupMessage(@NotNull Bot bot, @NotNull OnebotEvent.GroupMessageEvent event) {
        ...
        PreciseMessage preciseMessage = preciseService.getByKeyword(text);
        if (!(preciseMessage == null)) {
            bot.sendGroupMsg(groupId, preciseMessage.getResponse(), false);
